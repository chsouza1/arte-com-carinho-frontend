...
export function LoginForm() {
  ...
  const loginMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      // NOVO: Tratamento específico do 429
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (loginMutation.isPending) return; // Proteção extra contra múltiplos cliques
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button 
              ...
              disabled={loginMutation.isPending}
              className="w-full h-10 gap-2 rounded-sm border border-[#D7CCC8] hover:bg-[#FAF7F5] text-[#5D4037] font-bold text-xs uppercase disabled:opacity-70"
              type="button"
            >
